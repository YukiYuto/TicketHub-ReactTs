...
export type FileType = 'avatar' | 'service' | 'service-combo';

// Upload file với FileType
export const uploadFile = async (file: File, fileType: FileType): Promise<UploadResponse> => {
  try {
...
export const uploadServiceImage = async (file: File): Promise<UploadResponse> => {
  return uploadFile(file, 'service');
};

// Upload service combo image (wrapper)
export const uploadServiceComboImage = async (file: File): Promise<UploadResponse> => {
  return uploadFile(file, 'service-combo');
};
